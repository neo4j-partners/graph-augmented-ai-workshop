...
2. MAS endpoint connectivity and gap analysis query
...
class DatabricksResponsesLM(dspy.BaseLM):
    ...
    def forward(self, prompt=None, messages=None, **kwargs):
        ...
        MAS endpoints only support single-turn conversations, so we combine
        multi-turn messages (generated by DSPy's ChatAdapter) into one
        user message before sending.
...
def query_mas(endpoint_name: str) -> str:
...
def main():
    ...
    print("=" * 60)
    print(f"  MAS Endpoint: {args.mas_endpoint}")
    print()
    ...
    try:
        ...
        record(
            "dspy_config", True,
            f"BaseLM(model_type=responses), endpoint={args.mas_endpoint}",
        )
    except Exception as e:
    ...
    print("\nStep 3: Query MAS for Gap Analysis")
    print("  (This may take 1-3 minutes as MAS routes to multiple agents...)")
    gap_analysis = ""
    try:
        t0 = time.time()
        gap_analysis = query_mas(args.mas_endpoint)
        ...
        record("mas_gap_analysis", False, str(e))
        _print_summary(results)
        sys.exit(1)
